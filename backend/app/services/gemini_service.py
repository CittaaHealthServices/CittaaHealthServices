...
# Gemini 3 model configuration - Using latest models for authentic analysis
GEMINI_MODEL = "gemini-3-pro-preview"
GEMINI_FLASH_MODEL = "gemini-3-flash-preview"

# Voice analysis scoring prompt for authentic clinical assessment
VOICE_ANALYSIS_PROMPT = """You are an expert clinical psychologist AI specializing in voice-based mental health assessment for CITTAA Health Services' Vocalysis system. Analyze the following voice biomarkers and provide authentic clinical scoring.

## Voice Biomarkers Extracted:
- **Pitch Mean**: {pitch_mean:.1f} Hz (Normal range: 100-250 Hz for adults)
- **Pitch Variability (Std)**: {pitch_std:.1f} Hz (Higher = more emotional expression)
- **Pitch Coefficient of Variation**: {pitch_cv:.3f}
- **Speech Rate**: {speech_rate:.2f} syllables/sec (Normal: 3-4 syl/sec)
- **Energy/Volume (RMS)**: {rms_mean:.4f} (Higher = louder speech)
- **Energy Variability**: {rms_std:.4f}
- **Jitter (Pitch Perturbation)**: {jitter:.4f} (Higher = vocal instability)
- **Shimmer (Amplitude Perturbation)**: {shimmer:.4f} (Higher = voice tremor)
- **Harmonics-to-Noise Ratio (HNR)**: {hnr:.1f} dB (Higher = clearer voice)
- **Spectral Centroid**: {spectral_centroid:.1f} Hz (Higher = brighter/tenser voice)
- **Pause Rate**: {pause_rate:.2f} (Higher = more pauses)
- **Speaking Ratio**: {speaking_ratio:.2f} (Lower = more silence)
- **Voice Tremor Index**: {voice_tremor:.4f}
- **Rhythm Regularity**: {rhythm_regularity:.3f}

## Clinical Research Correlations:
- **Anxiety**: High pitch variability, fast speech, high spectral centroid, voice tremor
- **Depression**: Low pitch, slow speech, low energy, monotone (low pitch std), high pause rate
- **Stress**: High jitter/shimmer, low HNR, irregular rhythm

## Instructions:
Based on these voice biomarkers and clinical research, provide authentic probability scores (0.0 to 1.0) for each condition. The scores must sum to 1.0.

IMPORTANT: Provide AUTHENTIC scores based on the actual voice features. Do NOT artificially smooth or bias the results. If the voice shows clear signs of a condition, reflect that in the scores.

Respond ONLY with a JSON object in this exact format:
{{
  "normal": <float 0.0-1.0>,
  "anxiety": <float 0.0-1.0>,
  "depression": <float 0.0-1.0>,
  "stress": <float 0.0-1.0>,
  "confidence": <float 0.0-1.0>,
  "clinical_notes": "<brief clinical observation>"
}}
"""
...
"""
    ...
    async def analyze_voice_features(self, voice_features: Dict[str, Any]) -> Dict[str, Any]:
        """
        Use Gemini 3 to analyze voice features and provide authentic clinical scoring.
        
        Args:
            voice_features: Extracted voice biomarkers from audio analysis
            
        Returns:
            Dictionary containing Gemini-enhanced probability scores and clinical notes
        """
        import json
        ...
        feature_data = {
            ...
            "pitch_cv": voice_features.get("pitch_cv", 0.2),
            "speech_rate": voice_features.get("speech_rate", 3.0),
            "rms_mean": voice_features.get("rms_mean", 0.1),
            "rms_std": voice_features.get("rms_std", 0.05),
            "jitter": voice_features.get("jitter_mean", 0.02),
            "shimmer": voice_features.get("shimmer_mean", 0.05),
            "hnr": voice_features.get("hnr", 15),
            "spectral_centroid": voice_features.get("spectral_centroid_mean", 2000),
            "pause_rate": voice_features.get("pause_rate", 0.5),
            "speaking_ratio": voice_features.get("speaking_ratio", 0.7),
            "voice_tremor": voice_features.get("voice_tremor", 0),
            "rhythm_regularity": voice_features.get("rhythm_regularity", 0.5),
        }
        
        formatted_prompt = VOICE_ANALYSIS_PROMPT.format(**feature_data)
        ...
        if self.is_available():
            try:
                ...
                probs = [
                    float(result.get("normal", 0.25)),
                    float(result.get("anxiety", 0.25)),
                    float(result.get("depression", 0.25)),
                    float(result.get("stress", 0.25))
                ]
