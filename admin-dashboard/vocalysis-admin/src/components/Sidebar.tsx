    ...
    <aside className="w-64 bg-white border-r border-gray-200 flex flex-col">
      ...
      <div className="p-6 border-b border-gray-200">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-gradient-to-br from-[#8B5A96] to-[#7BB3A8] rounded-xl flex items-center justify-center">
            <Mic className="w-5 h-5 text-white" />
          </div>
          <div>
            <h1 className="text-lg font-bold text-[#8B5A96]" style={{ fontFamily: 'cursive' }}>
              Cittaa
            </h1>
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all ${
                isActive
                  ? 'bg-[#8B5A96]/10 text-[#8B5A96] border-l-4 border-[#8B5A96]'
                  : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
              }`}
            >
              <Icon className={`w-5 h-5 ${isActive ? 'text-[#8B5A96]' : 'text-gray-400'}`} />
              {item.label}
            </button>
          )
        })}
